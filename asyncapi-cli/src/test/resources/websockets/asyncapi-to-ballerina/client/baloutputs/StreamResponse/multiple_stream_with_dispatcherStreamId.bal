...
public client isolated class ChatClient {
    ...
    remote isolated function doTesting(Testing testing, decimal timeout) returns stream<NextMessage,error?>|error {
        lock {
            if !self.isActive {
                return error("[doTesting]ConnectionError: Connection has been closed");
            }
        }
        pipe:Pipe self  INVALID[.] INVALID[pipes] INVALID[.] INVALID[getPipe] INVALID[(] INVALID["testing"] INVALID[)]= new (10000);
        string id;
        lock {
        ...
        if message is error {
            self.attemptToCloseConnection();
            return error("[doTesting]DataBindingError: Error in cloning message");
        }
        pipe:Error? pipeErr = self.writeMessageQueue.produce(message, timeout);
        if pipeErr is pipe:Error {
            self.attemptToCloseConnection();
            return error("[doTesting]PipeError: Error in producing message");
        }
        stream<NextMessage,error?> streamMessages;
        lock {
            NextMessageStreamGenerator streamGenerator = new (self.pipes.getPipe("testing"), timeout);
