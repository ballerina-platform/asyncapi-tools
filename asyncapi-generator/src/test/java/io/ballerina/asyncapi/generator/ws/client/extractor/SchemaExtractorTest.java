 ...
 * components with schemas, no components section, and components with empty schemas.
 */
public class SchemaExtractorTest {
    ...
    void testWithSchemas() throws AsyncApiParserException, GeneratorException {
        ...
        Assert.assertFalse(schemas.isEmpty(),
                "Should return a non-empty schema map when components.schemas exist");
        Assert.assertTrue(schemas.containsKey("MyEvent"),
                "Schema map should contain the 'MyEvent' entry defined in components.schemas");
    }
    ...
    void testWithNoComponents() throws AsyncApiParserException, GeneratorException {
        ...
        Assert.assertTrue(schemas.isEmpty(),
                "Should return an empty map when no components section exists in the spec");
    }
    ...
    void testWithEmptyComponentsSection() throws AsyncApiParserException, GeneratorException {
        ...
        Map<String, AsyncApiSchema> schemas = new SchemaExtractor(spec).extract();
        Assert.assertTrue(schemas.isEmpty(),
                "Should return an empty map when the components section contains no schemas");
    }
}
