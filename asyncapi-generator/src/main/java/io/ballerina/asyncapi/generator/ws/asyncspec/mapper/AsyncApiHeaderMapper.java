...
/**
 * Maps Ballerina header annotations to AsyncAPI message header objects.
 * Corresponds to the legacy {@code AsyncApiHeaderMapper}.
 */
public class AsyncApiHeaderMapper {
     ...
     * @param bindingHeaderObject target schema to add header property to
     */
    public void setHeaderParameter(RequiredParameterNode headerParam, BalAsyncApi30SchemaImpl bindingHeaderObject) {
        ...
        if (extractedHeaderName != null) {
            ...
            BalAsyncApi30SchemaImpl headerTypeSchema =
     ...
     * @param bindingHeaderObject target schema to add header property to
     */
    public void setHeaderParameter(DefaultableParameterNode headerParam, BalAsyncApi30SchemaImpl bindingHeaderObject) {
        String headerName = extractHeaderName(headerParam);
        BalAsyncApi30SchemaImpl headerTypeSchema = ConverterCommonUtils.getAsyncApiSchema(getHeaderType(headerParam));
        ...
        if (allowedTypes.contains(headerParam.expression().kind())) {
            ...
            headerTypeSchema = new BalAsyncApi30SchemaImpl();
            headerTypeSchema.setDefault(new TextNode(defaultValue));
        }
     ...
     */
    private void completeHeaderParameter(String headerName, BalAsyncApi30SchemaImpl headerSchema,
        ...
        if (node instanceof ArrayTypeDescriptorNode) {
            ...
            BalAsyncApi30SchemaImpl itemSchema = ConverterCommonUtils.getAsyncApiSchema(kind);
    ...
    private void enableHeaderRequiredOption(Node node, BalAsyncApi30SchemaImpl headerSchema) {
