...
// DEVIATION FROM PRODUCTION (documented in SUMMARY.md): points at the local mock server
// (testing-artifacts/google.mail/mock/) instead of the real https://pubsub.googleapis.com/v1,
// since that base URL is otherwise hardcoded on this locally-constructed http:Client with no
// config-based way to redirect it.
# Holds the value for URL of pubsub.
const string PUBSUB_BASE_URL = "http://localhost:8091/pubsub/v1";
...
// DEVIATION FROM PRODUCTION (documented in SUMMARY.md): points at the local mock server instead
// of the real https://gmail.googleapis.com/gmail, same reasoning as PUBSUB_BASE_URL above.
# Holds the value for the Gmail API base URL.
const string GMAIL_BASE_URL = "http://localhost:8091/gmail";
